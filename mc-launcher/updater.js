...
// Repair multi-layer UTF-8 → Windows-1252 mojibake in text fields
...
function stringToCP1252Bytes(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    return null; // unmappable character — not a CP1252 mojibake
  }
...
function sanitizeModpackText(mp) {
  const TEXT_FIELDS = ["name", "subtitle", "description"];
  const patched = { ...mp };
  for (const key of TEXT_FIELDS) {
    if (typeof patched[key] === "string") patched[key] = repairMojibake(patched[key]);
  }
  return patched;
}
...
function normalizeManifest(data) {
  ...
  if (data.formatVersion === 2 && Array.isArray(data.modpacks)) {
    return {
      ...data,
      modpacks: data.modpacks.map(sanitizeModpackText),
      launcher: {
